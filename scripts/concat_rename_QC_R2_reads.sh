...
# goal: concatenate, rename, and QC fastq files by sample (parent directory)

# create variable for source and target directories
origin_dir="/data/NHGRIBaxevanis/podo_data_sharing/Hydractinia_PopGen_reads/round2c_newengland"
...
samples=$(cat ${prodir}/data/samples_r2c.txt)

# loop to submit separate jobs for each sample read concatenation
for sample in $samples
do echo $sample
# create job file
JOBFILE="${prodir}/jobs/concat_QC_R2_${sample}.job"
touch $JOBFILE
